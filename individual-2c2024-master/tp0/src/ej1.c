...
{
  char* mon = moneda ? "ARG":"USD";
  fprintf(pfile,"%s,%d,%d,%d",productos[id],precio,mon,stock);
}

void procesar_e_imprimir_catalogo(FILE *pfile)
{
	uint32_t  maskID1 = 0xFF000000;
	uint32_t  maskID2 = 0x00FF0000;
	uint32_t  maskP1 = 0x00FFF000;
	uint32_t  maskM1 = 0x00000008;
	uint32_t  maskP2v1 = 0x00000007;
	uint32_t  maskP2v2 = 0xFF800000;
	uint32_t  maskM2 = 0x00400000;
	uint32_t  maskStock1 = 0x003FF800;
	uint32_t  maskStock2 = 0x000007FF;
	uint32_t id1,id2,precio1;

	for(int i=0;i<=32;i+2){
		if(i%2==0){
			
			//id1 = maskID1 & catalogo[i];
			//id2 = maskID2 & catalogo[i];
			//precio1 = mask3 & catalogo[i];
